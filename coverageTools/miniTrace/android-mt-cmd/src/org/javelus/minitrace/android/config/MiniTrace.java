...
public class MiniTrace {
    ...
    private int uid;
    private String[] harvestCmdArray;

    private File outputDirectory;

    private void ensureExist(File directory) {
        if (directory.exists()) {
            return;
        }
        if (!directory.mkdirs()) {
            System.out.println("Cannot make directory: " + directory);
        }
    }
    ...
    public MiniTrace(IActivityManager mAm, String packageName, int uid, File outputDirectory) {
        ...
        if (getIsArtInUse()) {
            harvestCmdArray = "su root kill -USR2 100".split(" ");
        } else {
            harvestCmdArray = "su root kill -USR1 100".split(" ");
        }
    }


    static String[] SETENFORCE_0 = "su root setenforce 0".split(" ");
    ...
    public void setup() {
        ...
        try {
            ...
            if (ret != 0) {
                System.out.println("WARNNING: setenforce 0 returns " + ret);
            }
            for (String config : CONFIG_FILES) {
                String fileName = String.format(config, uid);
                String remote = "/sdcard/" + fileName;
                String data = "/data/" + fileName;
                if (clearFile(remote)) {
                    ret = moveData(remote, data);
                    if (ret != 0) {
                        throw new RuntimeException("Cannot move " + remote + " to " + data);
                    }
                    ...
                    throw new RuntimeException("Cannot create file " + remote);
                }
            }
    ...
    public void tearDown() {
        ...
        try {
            ...
            for (String config : CONFIG_FILES) {
                ...
                String fileName = String.format(config, uid);
                ensureExist(outputDirectory);
                String remote = new File(outputDirectory, fileName).getAbsolutePath();
                String data = "/data/" + fileName;
                if (!checkExists(data)) {
                    throw new RuntimeException("Data " + data + " does not exist.");
                }
                ret = moveData(data, remote);
                if (ret != 0) {
                    throw new RuntimeException("Cannot move " + data + " to " + remote);
                }
            }
        } catch (RuntimeException e) {
            throw e;
    ...
    private int chownToUser(String fileName, int uid) {
        ...
        try {
            ret = executeCommandAndWaitFor(new String[] {
                    "su", "root", "chown", userGroup, fileName
            });
        } catch (Exception e) {
            e.printStackTrace();
        }
